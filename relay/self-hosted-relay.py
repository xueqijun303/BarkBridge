...
def chat_page(secret, selected):
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,viewport-fit=cover"><title>BarkBridge Chat</title><style>{CHAT_CSS}</style></head><body><div class="app"><header><div class="topbar"><h1 id="title">BarkBridge Chat</h1><nav><a class="settings" href="/admin?secret={esc(secret)}">管理</a><a class="settings" href="/settings?secret={esc(secret)}">设置</a></nav></div><div class="searches"><input id="search" class="search" placeholder="搜索联系人"><input id="messageSearch" class="search" placeholder="搜索消息内容"></div><div class="hint" id="resultHint">只显示 BarkBridge 启用后经手的完整消息</div></header><nav class="contacts" id="contacts"></nav><div class="filters" id="filters"><button data-filter="all" class="active" type="button">全部</button><button data-filter="incoming" type="button">收到</button><button data-filter="outgoing" type="button">发送</button><button data-filter="voice" type="button">语音</button><button data-filter="failed" type="button">失败</button></div><div class="chat-tools"><button id="latestChat" class="ghost primary" type="button">回到最新</button><button id="pinContact" class="ghost" type="button">置顶</button><button id="hideContact" class="ghost" type="button">移除</button><button id="clearChat" class="ghost" type="button">清空</button></div><main class="messages" id="messages"></main><form id="form"><textarea id="text" placeholder="输入回复内容"></textarea><button class="send" type="submit">发送</button></form></div><script>const secret={json.dumps(secret)};let selected={json.dumps(selected)};{CHAT_JS}</script></body></html>"""
...
.app{height:100dvh;min-height:100svh;display:grid;grid-template-rows:auto auto auto auto minmax(0,1fr) auto;overflow:hidden}
...
.topbar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center}.topbar nav{display:flex;gap:12px}.settings{color:var(--green);font-size:15px;font-weight:800;text-decoration:none}.ghost{height:38px;border:1px solid #46505a;border-radius:8px;background:#2a2d31;color:var(--text);font:15px inherit;font-weight:800;padding:0 12px}.searches{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.search{width:100%;height:42px;border:1px solid var(--line);border-radius:8px;background:#2a2d31;color:var(--text);font:17px inherit;padding:0 12px}
.contacts{display:flex;gap:8px;overflow:auto;padding:10px 12px;background:var(--panel);border-bottom:1px solid var(--line)}
.filters{display:flex;gap:8px;overflow:auto;background:var(--panel);border-bottom:1px solid var(--line);padding:8px 12px}.filters button{height:34px;border:1px solid var(--line);border-radius:999px;background:#2a2d31;color:var(--text);font:14px inherit;font-weight:800;padding:0 12px;white-space:nowrap}.filters button.active{background:var(--green);border-color:var(--green);color:#03150a}
.chat-tools{display:grid;grid-template-columns:112px 1fr 1fr 1fr;gap:8px;background:var(--panel);border-bottom:1px solid var(--line);padding:8px 12px}.chat-tools .ghost{width:100%;min-width:0}.ghost.primary{background:var(--green);border-color:var(--green);color:#03150a}
...
@media(max-width:560px){.searches{grid-template-columns:1fr}.chat-tools{grid-template-columns:1fr 1fr 1fr 1fr}.ghost{font-size:14px;padding:0 6px}}
